...
ul_psw_ = [4020, 5020, 2020, 6020]
ul_ipgw_ = [2145, 3145]
dl_psw_ = [3189, 2189, 4189]
dl_ipgw_ = [4215, 1215, 2215, 3215]
...
p4.SwitchIngress.ingress_dt_table.ul_psw_table.add_with_ul_psw(
    psw_u_start=66, psw_u_end=178, code=5020)

p4.SwitchIngress.ingress_dt_table.ul_psw_table.add_with_ul_psw(
    psw_u_start=178, psw_u_end=418, code=2020)

p4.SwitchIngress.ingress_dt_table.ul_psw_table.add_with_ul_psw(
    psw_u_start=419, psw_u_end=10000, code=6020)
...
p4.SwitchIngress.ingress_dt_table.ul_ipgw_table.add_with_ul_ipgw(
    ipgw_u_start=17417, ipgw_u_end=65000, code=3145)
...
p4.SwitchIngress.ingress_dt_table.dl_psw_table.add_with_dl_psw(
    psw_d_start=812, psw_d_end=909, code=2189)

p4.SwitchIngress.ingress_dt_table.dl_psw_table.add_with_dl_psw(
    psw_d_start=909, psw_d_end=1150, code=4189)
...
p4.SwitchIngress.ingress_dt_table.dl_ipgw_table.add_with_dl_ipgw(
    ipgw_d_start=2199, ipgw_d_end=9361, code=3215)
...
for dl_ipgw in range(0, 2):
    sum = dl_ipgw_[dl_ipgw] + ul_psw_[0] + dl_psw_[0]
    if sum in sum_code_:
        continue
    else:
        p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
        sum_code_.append(sum)

for dl_ipgw in range(0, 2):
    for ul_psw in range(0, 2):
        for dl_psw in range(0, 2):
            sum = ul_psw_[ul_psw] + dl_psw_[dl_psw] + dl_ipgw_[dl_ipgw]
            if sum in sum_code_:
                continue
            else:
                p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
...
for dl_ipgw in range(0, 2):
    for ul_psw in range(0, 2):
        for ul_ipg in range(0, 2):
            sum = ul_psw_[ul_psw] + ul_ipgw_[ul_ipg] + dl_ipgw_[dl_ipgw]
            if sum in sum_code_:
                continue
            else:
                p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
...
for ul_psw in range(0, 3):
    for dl_ipgw in range(0, 3):
        sum = ul_psw_[ul_psw] + dl_ipgw_[dl_ipgw]
        if sum in sum_code_:
            continue
        else:
            p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
...
for ul_psw in range(1, 3):
    for ul_ipgw in range(0, 2):
        for dl_psw in range(0, 3):
            for dl_ipgw in range(0, 4):
                sum = ul_psw_[ul_psw] + ul_ipgw_[ul_ipgw] + dl_ipgw_[dl_ipgw] + dl_psw_[dl_psw]
                if sum in sum_code_:
                    continue
                else:
                    p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
...
for ul_psw in range(1, 3):
    for ul_ipgw in range(0, 2):
        for dl_psw in range(0, 3):
            sum = ul_psw_[ul_psw] + ul_ipgw_[ul_ipgw] + dl_ipgw_[0] + dl_psw_[dl_psw]
            if sum in sum_code_:
                continue
            else:
                p4.SwitchIngress.ingress_dt_table.is_cg_table.add_with_is_cg(sum_code=sum)
...
print ("\nTable lpfIPGwD:")
p4.SwitchIngress.lpfIPGwU.dump(table=True)

print ("\nTable lpfPSwU:")
p4.SwitchIngress.lpfPSwU.dump(table=True)

print ("\nTable lpfIPGwD:")
p4.SwitchIngress.lpfIPGwD.dump(table=True)

print ("\nTable lpfPSwD:")
p4.SwitchIngress.lpfPSwD.dump(table=True)
